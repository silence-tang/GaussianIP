...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        if hasattr(self, 'smpl_opt') and self.smpl_opt is not None:
            l.append({'params': self.smpl_opt.beta, 'lr': 0.004, 'name': 'smpl_opt.beta'})
    ...
    def save_ply(self, path):
        ...
        lowest_x = min(xyz, key=lambda coord: coord[0])
        lowest_y = min(xyz, key=lambda coord: coord[1])
        lowest_z = min(xyz, key=lambda coord: coord[2])
