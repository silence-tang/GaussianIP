...
class Skeleton:
    def __init__(self, smplx_path, gender='neutral', style='openpose', apose=True):
        self.style = style
        self.apose = apose
        self.device = 'cuda'
        ...
        # NOTE: load smplx model, 注意这边是使用的custom版本, 而不是以前的
    ...
    def forward_smplx(self, betas=None, expression=None, body_pose=None):
        body_pose = np.zeros((21, 3), dtype=np.float32) if body_pose is None else body_pose
        ...
        # NOTE: coordinate system: opengl --> blender (switch y/z)
        self.points3D[:, [1, 2]] = self.points3D[:, [2, 1]]
        self.vertices[:, [1, 2]] = self.vertices[:, [2, 1]]
