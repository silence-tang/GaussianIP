...
def draw_humansd_skeleton(image, pose, mmpose_detection_thresh=0.3, height=None, width=None, humansd_skeleton_width=10):
    ...
    def plot_kpts(img_draw, kpts, color, edgs, width):
        for idx, kpta, kptb in edgs:
            # kpts: [17,3]
            if kpts[kpta, 2] > mmpose_detection_thresh and kpts[kptb, 2] > mmpose_detection_thresh:
...
class Skeleton:
    def __init__(self, style='openpose', apose=False):
        self.style = style
        self.apose = apose
    ...
    def load_smplx(self, path, betas=None, expression=None, gender='neutral'):
        ...
        if self.style == "humansd":
            joints = joint_mapper_smplx_to_humansd17(joints)            # [17, 3]
        else:
            joints = joint_mapper_smplx_to_openpose18(joints)
            
        self.points3D = np.concatenate([joints, np.ones_like(joints[:, :1])], axis=1) # [17, 4]
    ...
    # controlnet takes openpose map as input condition
    def openpose_draw(self, mvp, H, W, azimuth, head_zoom, enable_occlusion):
        # mvp: [4, 4]
        canvas = np.zeros((H, W, 3), dtype=np.uint8)

        points = self.points3D @ mvp.T # [18, 4]
        points = points[:, :3] / points[:, 3:] # NDC in [-1, 1]
        xs = (points[:, 0] + 1) / 2 * W # [18]
        ys = (points[:, 1] + 1) / 2 * H # [18]
        ...
        if enable_occlusion:
            ...
            if points[0, 2] > points[-1, 2] and points[0, 2] < points[-2, 2]:
                ...
                mask[-4] = False    # no right eye
                if azimuth < 0:
                    mask[-3] = False

            elif points[0, 2] < points[-1, 2] and points[0, 2] > points[-2, 2]:
                mask[-1] = False    # no left ear
                mask[-3] = False    # no left eye
        ...
        all_vis = 1 if all(mask) else 0

        return canvas, all_vis, np.stack([xs, ys], axis=1)
