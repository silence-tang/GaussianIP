    ...
    def get_text_embeddings(
        ...
        if view_dependent_prompting:
            ...
            for d in self.directions:
                direction_idx[d.condition(elevation, azimuth, center, all_vis_all, camera_distances)] = self.direction2idx[d.name]
                # direction_idx[d.condition(elevation, azimuth, center, camera_distances)] = self.direction2idx[d.name]
        ...
        null_embeddings = self.null_embeddings.expand(batch_size, -1, -1)
...
class PromptProcessor(BaseObject):
    ...
    class Config(BaseObject.Config):
        prompt: str = "a man wearing black jacket"
        negative_prompt: str = ""
        negative_prompt_faceid: str = ""
        null_prompt: str = ""
    ...
    def configure(self) -> None:
        ...
        if self.cfg.view_dependent_prompt_front:
            self.directions = [
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, cent, dis: ((azi > -45) & (azi < 60)) | (azi < -135) | (azi > 120),
                ),
                ...
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, cent, dis: (azi > 60) & (azi < 120),
                ),
                ...
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, cent, dis: (azi > -135) & (azi < -45),
                ),
                DirectionConfig(
                    "overhead",
                    lambda s: f"overhead view of {s}",
                    lambda s: s,
                    lambda ele, azi, cent, dis: (cent == self.cfg.head_offset) & (azi > 0),
                ),
            ]
        else:
            self.directions = [
                DirectionConfig(
                    "left front",
                    lambda s: f"{s}, left front view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 0) & (azi > 0) & (azi < 45),
                ),
                DirectionConfig(
                    "left back",
                    lambda s: f"{s}, left back view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 0) & (azi > -45) & (azi < 0),
                ),
                DirectionConfig(
                    "right front",
                    lambda s: f"{s}, right front view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 0) & (azi > 135),
                ),
                DirectionConfig(
                    "right back",
                    lambda s: f"{s}, right back view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 0) & (azi < -135),
                ),
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 0) & (azi > 45) & (azi < 135),
                ),
                DirectionConfig(
                    "back",
                    lambda s: f"{s}, back view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 0) & (azi > -135) & (azi < -45),
                ),
                ##############################
                DirectionConfig(
                    "left front",
                    lambda s: f"{s}, full body photo, left front view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 1) & (azi > 0) & (azi < 45),
                ),
                DirectionConfig(
                    "left back",
                    lambda s: f"{s}, full body photo, left back view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 1) & (azi > -45) & (azi < 0),
                ),
                DirectionConfig(
                    "right front",
                    lambda s: f"{s}, full body photo, right front view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 1) & (azi > 135),
                ),
                DirectionConfig(
                    "right back",
                    lambda s: f"{s}, full body photo, right back view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 1) & (azi < -135),
                ),
                DirectionConfig(
                    "front",
                    lambda s: f"{s}, full body photo, front view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 1) & (azi > 45) & (azi < 135),
                ),
                DirectionConfig(
                    "back",
                    lambda s: f"{s}, full body photo, back view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (all_vis == 1) & (azi > -135) & (azi < -45),
                ),
                DirectionConfig(
                    "overhead",
                    lambda s: f"{s}, overhead view",
                    lambda s: s,
                    lambda ele, azi, cent, all_vis, dis: (cent == self.cfg.head_offset) & (azi > 0),
                )
            ]
        ...
        self.negative_prompt_faceid = self.cfg.negative_prompt_faceid
        self.null_prompt = self.cfg.null_prompt

        threestudio.info(
            f"Using prompt [{self.prompt}] and negative prompt [{self.negative_prompt}] for ip-adapter"
        )
        threestudio.info(
            f"Using prompt [{self.prompt}] and negative prompt [{self.negative_prompt_faceid}] for ip-adapter-faceid"
        )
        ...
        if self.cfg.use_prompt_debiasing:
            ...
            self.prompts_vd = [self.cfg.get(f"prompt_{d.name}", None) or d.prompt(self.prompt) for d in self.directions]
    ...
    def prepare_text_embeddings(self):
        ...
        if self.cfg.use_ipa_faceid:
            self.pretrained_model_name_or_path = self.cfg.pretrained_realistic_model_name_or_path
        else:
            self.pretrained_model_name_or_path = self.cfg.pretrained_sd_model_name_or_path

        for prompt in all_prompts:
            if self.cfg.use_cache:
                # some text embeddings are already in cache
                # do not process them
                cache_path = os.path.join(self._cache_dir, f"{hash_prompt(self.pretrained_model_name_or_path, prompt)}.pt")
                if os.path.exists(cache_path):
                    threestudio.debug(f"Text embeddings for model {self.pretrained_model_name_or_path} and prompt [{prompt}] are already in cache, skip processing.")
                    continue
    ...
    def load_text_embeddings(self):
        ...
        self.uncond_text_embeddings_vd = torch.stack([self.load_from_cache(prompt) for prompt in self.negative_prompts_vd], dim=0)
